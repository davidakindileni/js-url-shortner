...
// Project Description: A project that receives a URL link, shortens it, stores it in a localStorage array,
//     and allows the user to retrieve the long URL used.
...
// URL database
// Retrieve the saved array from localStorage, or create a new one is none exists.
...
// If a generated short URL already exists, the function calls itself again
// using recursion.
function generateUniqueShortUrl() {
    const shortCode = nanoid(6).toUpperCase();
    const shortUrl = `https://${shortCode}`;

    // find() checks whether the generated URL already exists in the array.
...
function validateUrl(url) {
    ...
    // Error catching routine for invalid URLs
    try {
        new URL(url);
    } catch {
        throw new Error(
...
function clearFields() {
    input.value = "";
    shortUrlInput.value = "";
    // Delete should only become active when an existing URL has been selected.
    deleteButton.disabled = true;
    // Return focus to the long URL field.
    input.focus();
}

// Display existing, stored URLs
function displayUrls() {
    ...
    const urlItems = urlDatabase.map(function (url) {
        ...
        // Make the shortened URL an anchor link
        const shortLink = document.createElement("a");
        shortLink.href = "#";
        shortLink.textContent = url.shortUrl;

        // Retrieve the long URL when the short URL is clicked
        ...
        // Create the "X" button for deleting line items
        ...
        // function to delete short URL line items
        ...
        // Add the short URL and X button to the list item.
...
// Delete the displayed long and short URL
